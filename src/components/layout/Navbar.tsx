    ...
    <nav className="bg-dark-900 border-b border-dark-800 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-6 py-3.5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <SignedOut>
            <Link href="/login"
              className="text-sm text-dark-400 hover:text-white transition-colors font-medium">
              Connexion
            </Link>
            <Link href="/login"
          ...
          <SignedIn>
            <Link href="/dashboard"
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                pathname === '/dashboard' ? 'text-white bg-dark-800' : 'text-dark-400 hover:text-white hover:bg-dark-800'
              }`}>
              Dashboard
            </Link>
