...
import { Search, Calculator, FileSpreadsheet, TrendingUp, ChevronRight, Lock, ArrowUpRight, CheckCircle } from 'lucide-react'
import { isPremiumUser, upsertUser } from '@/lib/supabase'
...
export default async function DashboardPage() {
  ...
  // Créer l'utilisateur dans Supabase s'il n'existe pas encore
  await upsertUser(userId, email)

  const premium = await isPremiumUser(userId)
  const firstName = user?.firstName ?? email?.split('@')[0]

  // Message de succès après upgrade
  return (
    <div className="min-h-screen bg-slate-50">
          ...
          <div className="flex items-center gap-3">
            {premium && (
              <div className="flex items-center gap-1.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold px-3 py-1.5 rounded-full">
                <CheckCircle size={12} /> Premium actif
              </div>
            )}
            {!premium && (
              <Link href="/tarifs"
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {TOOLS.map(({ href, icon: Icon, bg, color, title, desc, premium: toolPremium }) => {
            const locked = toolPremium && !premium
            return (
        ...
        {!premium && (
          <div className="mt-8 bg-slate-900 rounded-2xl p-6 flex items-center justify-between gap-6">
            ...
            <Link href="/tarifs"
              className="flex-shrink-0 bg-sky-500 hover:bg-sky-600 text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition-colors">
              14 jours gratuits
            </Link>
          </div>
        )}
